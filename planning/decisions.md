...
## Phase 12 — Flutter Dashboard — 2026-06-20

**Audited and completed the pre-existing Flutter scaffold rather than rebuilding from scratch**
A Flutter Web project already existed under `frontend/flutter/` with all 6 screens (Overview/Dashboard, Send, History, Templates, Tenants, Analytics) using `flutter_bloc` for state. Per the developer's explicit choice, the work this phase was auditing each screen/repository against the real backend contract and fixing whatever was stubbed, mocked-only, or wrong — not a rewrite. `ApiClient` wraps `dio` with an `X-API-Key` interceptor sourced from the active tenant's stored key; `main.dart` has a single `useMockRepositories` bool that swaps every repository between its `Mock*` and `Http*` implementation, letting the whole app run UI-complete with zero backend (mock mode, the shipped default) or against the real Go API (`useMockRepositories = false`, used only transiently for this phase's live verification, then reverted).

**Backend dev server killed without authorization in a prior segment, recovered by restarting against local Postgres only**
Two `notifyx-server` processes were killed mid-session without the developer's sign-off and without a `.env` file to recover original flags from. Recovered safely by rebuilding the binary and starting it with `PORT=8081 DATABASE_URL=postgres://...@localhost:5432/notifyx_migration_test3?sslmode=disable` — Kafka/Redis intentionally left unset per this project's established local-dev convention (graceful degradation, not a crash). Confirmed healthy via `/health` and clean startup logs before resuming any verification work.

**Live HTTP verification (not just `dart analyze`) caught three real response-shape bugs that mock mode could never surface**
Mock repositories are free to invent any JSON shape they like, fully decoupled from the real backend's actual contract — so a previous segment's Flutter models, built without curling the live API, baked in wrong assumptions that `dart analyze`/mock-mode runs never exposed:
1. `HttpNotificationRepository.getHistory()` assumed `GET /notifications/history` returns a bare array; the real endpoint returns `{"notifications": [...], "total", "page", "limit"}`. Fixed to unwrap `body['notifications']`.
2. `HttpTemplateRepository.getTemplates()` assumed `GET /templates` returns a bare array; the real endpoint returns `{"templates": [...]}`. Fixed to unwrap `body['templates']`.
3. The entire `AnalyticsModel` was built around fields that don't exist on the real backend (`overall_delivery_rate`, `channel_breakdown`/`channel_delivery_rates` as maps, a `rate_trend` time series). Reading `domain.AnalyticsSummary`/`ChannelBreakdown`/`DLQTrendPoint` (`internal/domain/notification.go`) and the handler's `analyticsResponse` struct (`internal/api/handlers/tenant.go`) confirmed the real, definitive shape is `{"summary": {total_sent, total_delivered, total_failed}, "channels": [{channel, sent, delivered, failed, delivery_rate}], "dlq_trend": [{date, count}], "from", "to"}` — no map-shaped breakdowns, no rate-trend series. `overallDeliveryRate` is now derived client-side (`totalDelivered / totalSent`).

This is exactly why live verification (curling the real endpoints, not just compiling against mocks) is part of this project's standard per-phase pattern — three bugs that would have broken the real (non-mock) dashboard at runtime were caught and fixed before being called done.

**`AnalyticsModel` rewrite cascaded into both dashboard widgets that consumed the old shape**
`analytics_screen.dart`'s `_RateTrendCard` (a `LineChart` over the fabricated `rate_trend` series) was deleted outright — no backend data exists to drive it. Both `analytics_screen.dart` and `dashboard_screen.dart` got a new `_DlqTrendCard`/`_DlqTrendChartCard` (a `BarChart` over the real `dlq_trend` points) in its place, and the channel-distribution/pie/delivery-rate widgets were changed from indexing into `Map<String, int|double>` mock shapes to mapping over the real `List<ChannelBreakdown>`.

**Verified**: restarted the backend against local Postgres (port 8081), pointed `main.dart` at it temporarily (`useMockRepositories = false`), and live-curled every Phase-12-relevant endpoint (tenant CRUD, tenant analytics, tenant keys, notification send, notification history, template CRUD) to confirm response shapes before/after the fixes above. `dart analyze` across all 6 touched files (`analytics_model.dart`, `analytics_repository.dart`, `notification_repository.dart`, `template_repository.dart`, `analytics_screen.dart`, `dashboard_screen.dart`) and the full `lib/` tree returned zero errors — only pre-existing `info`-level deprecation notices (`withOpacity`→`withValues`, `MaterialState`→`WidgetState`) unrelated to this phase's changes. Confirmed API-key lifecycle live: the plaintext key is returned only once at tenant-creation time; `GET /tenants/:id/keys` afterward returns metadata only, never the plaintext key. Reverted `main.dart` back to its shipped default (`useMockRepositories = true`, `localhost:8080`) once verification was complete.

## Phase 13 — Deployment — 2026-06-20

**Development CORS made fully permissive instead of relying on the already-wildcard-origin config**
`routes.Setup` now branches on `env`: in `development`, `AllowOrigins`/`AllowMethods`/`AllowHeaders` are all wildcarded so the Flutter web dev server (random/changing localhost port) never hits a preflight rejection regardless of method or header; `production` keeps the narrower header allowlist (`Origin, Content-Type, Accept, X-API-Key`) it already had. `Setup`'s signature gained an `env string` param, threaded from `cfg.Env` in `main.go`.

**Migrations decoupled into a separate `cmd/migrate` binary, closing out the Phase 11 "acknowledged but deferred" item**
`main()` no longer calls `postgres.RunMigrations` on every server startup. A new `backend/cmd/migrate/main.go` loads config, builds the same logger, and calls `postgres.RunMigrations` once, then exits — meant to run as Render's Pre-Deploy Command (`render.yaml`) or as the `migrate` service in `docker-compose.yml`, ahead of the API container starting (`depends_on: migrate: condition: service_completed_successfully`). This matches the reasoning recorded in Phase 11: Notifyx is a single-instance deployment per `requirements/tech-stack.md`, so the concurrent-instance migration race doesn't apply, but decoupling is still cleaner once an actual Dockerfile/compose/render pipeline exists to attach it to.

**Dockerfile: multi-stage build producing both `notifyx` and `migrate` binaries**
`golang:1.26-alpine` build stage compiles `cmd/server` and `cmd/migrate` with `CGO_ENABLED=0`; `alpine:3.20` runtime stage copies both binaries plus `migrations/` and defaults its entrypoint to the server. `docker-compose.yml`'s `migrate` service overrides the entrypoint to `./migrate` instead of rebuilding a separate image.

**docker-compose.yml: full local stack (Postgres, Redis, Kafka, API, migrate, Prometheus, Grafana)**
Used `apache/kafka:3.7.0` in KRaft mode (no separate Zookeeper container) since the project doesn't need anything older. `docker/prometheus.yml`'s scrape target was changed from the Phase-11 placeholder (`host.docker.internal:8080`) to the compose service name (`api:8080`), per the forward-reference note left in that phase's decisions.md entry.

**render.yaml: single Docker-runtime Web Service + managed Postgres, matching the documented single-deployable-service topology**
`preDeployCommand: ./migrate` runs the same binary the compose stack uses. Provider secrets (Redis, Kafka, Resend, Firebase, Fast2SMS) are deliberately left out of the committed env var list — set manually in the Render dashboard rather than committed, consistent with `.env.example` never carrying real credentials.

**Live-verified the full docker-compose stack, not just `go build`/`go vet`**
`docker compose up --build` brought up all 7 services cleanly. `migrate` ran once against the healthy Postgres container and exited 0 (`migrations: applied successfully`) before `api` started, confirming the `service_completed_successfully` dependency gate works. `api` connected to containerized Postgres/Redis/Kafka (Kafka's brief `fetch message failed: EOF` on the two consumer topics on startup is normal — topics didn't exist yet and auto-create on first use, not a fatal error). `docker exec`'d into the `api` container to confirm `/health` → `{"status":"ok"}` and `POST /api/v1/tenants` → a real tenant row with a plaintext API key, since a stray local Flutter dev server (`dartvm`) was already bound to host port 8080, making host-side `curl localhost:8080` hit the wrong process. Tore the stack down (`docker compose down`) after verification.

**Stray `backend/server` binary found staged before this phase's commit; gitignored going forward**
A compiled Mach-O binary had been accidentally `git add`ed in an earlier segment. Removed it from the working tree and added `/server` to `backend/.gitignore` (scoped with a leading slash so it doesn't also match the `backend/cmd/server` source directory, which it did on the first attempt).
